  ...
  case RAD_PUB_TYPE_KINECT_DATA:
    pDepthGen =
      (xn::DepthGenerator*)((KinectDataPub*)RADFactory::getPtrToInst()->
        getPtrToPub(g_pubType))->getPrdNode(XN_NODE_TYPE_DEPTH);
    break;
  case RAD_PUB_TYPE_VIDEO_DATA:
    pDepthGen =
      (xn::DepthGenerator*)((VideoDataPub*)RADFactory::getPtrToInst()->
	getPtrToPub(g_pubType))->getPrdNode(XN_NODE_TYPE_DEPTH);
    break;
  default:
    printf("Unknown publisher type\n");
    exit(EXIT_FAILURE);
  }
