...
{
  rad::SensorDataPub* pPub;

  if (argc == 2 && strcmp(argv[1], "-v") == 0){
    pPub = rad::RADFactory::getPtrToInst()->
      getPtrToPub(rad::RAD_PUB_TYPE_VIDEO_DATA);

    g_pubType = rad::RAD_PUB_TYPE_VIDEO_DATA;
    printf("Run with video\n");
  }
  else{
    pPub = rad::RADFactory::getPtrToInst()->
      getPtrToPub(rad::RAD_PUB_TYPE_KINECT_DATA);

    g_pubType = rad::RAD_PUB_TYPE_KINECT_DATA;
  }
  ...
  while(!glfwWindowShouldClose(win)){
    ...
    if (g_pubType == rad::RAD_PUB_TYPE_VIDEO_DATA &&
	((rad::VideoDataPub*)pPub)->isEndOfVideo())
      break;
