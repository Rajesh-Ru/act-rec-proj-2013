...
private:
  ...
  void getUserDiscriptor(XnUserID id, const xn::DepthMetaData* pDepthMD,
			 const xn::SceneMetaData* pSceneMD,
			 cv::Mat &result);
  //  void getUserDiscriptor(const std::vector<JointInfo> &joints,
  //			 cv::Mat &relJoints);
  ...
  std::map<XnUserID, XnVector3D> startLeftHandPos, startRightHandPos,
    startNeckPos, preLeftHandPos, preRightHandPos;
