...
glmFindOrAddTexture(GLMmodel* model, const char* name)
{
    ...
    char *dir, *filename;
    float width, height;
	//printf("glmFindOrAddTexture invoke!\n");
    /* XXX doing a linear search on a string key'd list is pretty lame,
       but it works and is fast enough for now. */
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    GLuint blendmodel = 0;//是否启用混合
    GLMgroup* group;//模型的各部分
    GLMtriangle* triangle;//模型的面片
    GLuint material, map_diffuse;
    GLMmaterial* materialp;//模型材质的指针
    ...
    for(blenditer = 0; blenditer<2; blenditer++) {
		...
		while (group) {
			if (mode & (GLM_MATERIAL|GLM_COLOR|GLM_TEXTURE)) {
				material = group->material;//获得材质的ID

				materialp = &model->materials[material];//根据ID取得材质的指针
				...
				//printf("set %d blending:%f!\n",flag++,materialp->diffuse[3]);
				blendmodel |= blending;
				//printf("set blending:%d!\n",blendmodel);
				newmaterial = 1;
...
#endif
				/*if(map_diffuse == 1&&!(mode&GLM_TEXTURE)){
					break;
				}*/
