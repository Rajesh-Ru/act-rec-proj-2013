...
glmLoadTexture(const char *filename, GLboolean alpha, GLboolean repeat, GLboolean filtering, GLboolean mipmaps, GLfloat *texcoordwidth, GLfloat *texcoordheight)
{
    ...
    GLuint tex;//ÎÆÀíµÄID
    int width, height,pixelsize;//³¤¡¢¿í¡¢ÏñËØ³ß´ç
    int type;//ÀàÐÍ
    int filter_min, filter_mag;//ÂË²¨Æ÷
    GLubyte *data, *rdata;
    double xPow2, yPow2;
    int ixPow2, iyPow2;
    int xSize2, ySize2;
    GLint retval;
	printf("glmLoadTexture invoke !\n");
    if(glm_do_init){
		printf("glm init!\n");
		glmImgInit();
	}
...
#endif
#ifdef FREE_IMAGE
    data = glmReadFreeImage(filename, alpha, &width, &height, &type);
    if(data != NULL) {
	DBG_(__glmWarning("glmLoadTexture(): got image for %s",filename));
	goto DONE;
    }
#endif
...
/*#define FORCE_ALPHA*/
	...
	if (!rdata)
	    return 0;
	//printf("%d %d %d %d %d\n", type, width, height, xSize2, ySize2);
